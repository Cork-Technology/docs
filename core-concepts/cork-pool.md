...
`Cork Swap Token` or `cST`: a swap token that allows a user, before its expiry, to make the following exchange:
...
**Mint Mechanism**: The Collateral Asset is deposited and used to mint the Cork Principal Token and Cork Swap Token, which are returned to the depositor. In the diagram below, the flow can be seen with ETH as the Collateral Asset.
...
In the wstETH example above, it would compute as follows:
...
When the Cork Pool receives Reference Asset + Cork Swap Token during periods when swaps are exercised, these assets can be repurchased to facilitate potential arbitrage. The Reference Asset Repurchase Mechanism allows you to repurchase the Reference Asset + Cork Swap Token with the original amount of Collateral Asset (minus a fee that linearly decreases with time to expiry). The exchange is as follows:

$$
cST + REF = CA + \text{fee}
$$

There are two scenarios where one would do this. If the price of Cork Principal Token < Reference Asset, then both the Reference Asset and Cork Swap Token can be sold for Collateral Asset at a profit. Alternatively, if the price of Cork Swap Token + Reference Asset > Collateral Asset, then these are sold for Collateral Asset at a profit. Arbitrage bots will automatically execute such trades to keep the Cork Swap Token and Cork Principal Token in line with the Reference Asset price.
